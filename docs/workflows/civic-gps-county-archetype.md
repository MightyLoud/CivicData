...
- keep unmodeled countywide offices and any unreleased civic-action routing explicit as gaps rather than implying completeness;
...
Brazos's v0.1 geography/office package contains 18 canonical offices / 18 holders = 6 deliberately bounded countywide + 4 Commissioner + 4 JP + 4 Constable. All three district families share the official P1–P4 geometry and use the generic one-meter topology probe. Action routing v0.1 is `RELEASE_BACKED` with 23 verified official routes: normal interiors return 14, while an exact shared boundary suppresses the three ambiguous precinct routes and preserves 11 body/countywide routes. Additional countywide and judicial offices remain `BOUNDED_V0_1_SCOPE`.
