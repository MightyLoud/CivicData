#!/usr/bin/env python3
"""One-shot deterministic packager for Civic GPS v0.6.1 + Tarrant County."""
...
SPEC = ROOT / "tests" / "fixtures" / "civic_gps_county_onboarding" / "tarrant_county5_candidate_v0.1.json"
OUT = ROOT / "artifacts" / "civic-gps-tarrant-package"
GENERATED = OUT / "onboarding"
OLD_SHA = "52e60a83b42c65cd03bf81c3169c54c86d8c7750686d5d827838ec636b4e26de"
EXPECTED_SPEC_SHA = "7885a7887482790c995f9d19753fc7ab366ab836a17ab05b1038abb84015eab8"
...
def build_onboarding_candidate() -> tuple[dict, dict]:
    ...
    if report.get("decision") != "GO" or report.get("result") != "SUPPORTED_V0_1":
        raise SystemExit(f"Tarrant onboarding fit changed: {report}")
    if report.get("spec_sha256") != EXPECTED_SPEC_SHA:
        raise SystemExit(f"Tarrant frozen spec SHA changed: {report.get('spec_sha256')}")
    ...
    release["meta"]["note"] = (
        "Tarrant County is County #5 and the first production candidate generated through "
        "County Onboarding Pipeline v0.1. CG-01 through CG-08 passed on the unchanged Civic "
        "GPS v0.6.1 engine. The bounded release contains six countywide offices, Commissioner "
        "precincts 1-4, and Justice of the Peace / Constable precincts 1-8. Civic-action routing "
    ...
    for jurisdiction in release.get("payload", {}).get("jurisdictions", []):
        if jurisdiction.get("jurisdiction_id") == "jur-us-tx-tarrant-county":
    ...
    for gap in bundle.get("known_gaps", []):
        if gap.get("gap_id") == "GAP-TARRANT-GPS-003":
            gap["status"] = "PROTECTED_PROMOTION_PENDING"
            gap["summary"] = (
                "CG-01 through CG-08 passed; this deterministic v0.5.5 package is validated in "
...
def patch_registry_and_release() -> str:
    ...
    if any(row.get("adapter_id") == "ADAPTER-TX-TARRANT" for row in registry.get("bundles", [])):
        raise SystemExit("Tarrant is already present in the base registry")
    engine_sha = sha256(GPS / "engine.py")
    registry["registry_artifact_version"] = "0.5.5"
...
def patch_readme() -> None:
    ...
    text = text.replace("registry artifact v0.5.4", "registry artifact v0.5.5")
    text = text.replace(
        "The packaged registry now includes eight jurisdiction bundles:",
    ...
    text += (
        "\nTarrant County is County #5 and the first production candidate generated by County "
        "Onboarding Pipeline v0.1. Eight permanent interiors cover Commissioner precincts 1-4 "
        "and JP/Constable precincts 1-8; the Austin negative proves county isolation; and "
        "independent live-derived Commissioner and shared JP/Constable boundaries preserve "
        "unaffected office families while suppressing only ambiguous assignments. Tarrant action "
        "routing and additional countywide or judicial offices remain explicitly outside the "
        "bounded v0.1 release scope.\n"
    )
...
def verify_candidate(expected_engine_sha: str, runtime_sha: str) -> dict:
    if sha256(GPS / "engine.py") != expected_engine_sha:
        raise SystemExit("Civic GPS engine changed during Tarrant packaging")
    ...
    if not bundle or bundle.get("release_files") != [RELEASE_NAME]:
        raise SystemExit("Tarrant bundle/release linkage mismatch")
    if bundle.get("action_registry_files"):
        raise SystemExit("Tarrant actions must remain unreleased")
    adapters = bundle.get("district_adapters", [])
    if len(adapters) != 3:
        raise SystemExit(f"Tarrant must contain three district adapters, got {len(adapters)}")
    for adapter in adapters:
        if adapter.get("failure_scope") != "ADAPTER":
            raise SystemExit("Tarrant district adapters must remain ADAPTER-scoped")
        if adapter.get("boundary_policy") != BOUNDARY_POLICY:
            raise SystemExit("Tarrant boundary policy changed")
        if adapter.get("officeholder_identity_source") != "CANONICAL_RELEASE_ONLY":
            raise SystemExit("Tarrant identity source changed")
    offices = release.get("payload", {}).get("offices", [])
    holders = release.get("payload", {}).get("officeholders", [])
    if len(offices) != 26 or len(holders) != 26:
        raise SystemExit("Tarrant release must contain exactly 26 offices/officeholders")
    if {row.get("office_id") for row in offices} != {row.get("office_id") for row in holders}:
        raise SystemExit("Tarrant office/officeholder joins changed")
    release_without_hash = copy.deepcopy(release)
    ...
    if recorded_release_sha != canonical_sha(release_without_hash):
        raise SystemExit("Tarrant canonical content SHA mismatch")
    return {
        "status": "PASS",
        "gate": "CG-09",
        ...
        "tarrant_offices": 26,
        "tarrant_holders": 26,
        "tarrant_actions": "NOT_YET_RELEASED",
...
def main() -> int:
    ...
    print(f"TARRANT_RUNTIME_SHA={first}")
    return 0
