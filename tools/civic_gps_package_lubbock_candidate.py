#!/usr/bin/env python3
"""Build the deterministic Lubbock County CG-09 candidate without mutating production."""
...
SPEC_PATH = ROOT / "tests/fixtures/civic_gps_county_onboarding/lubbock_county13_candidate_v0.1.json"
ONBOARDING_TOOL = ROOT / "tools/civic_gps_county_onboarding.py"
RUNTIME_PARTS = ROOT / "civic_gps_runtime_parts"
BASE_RUNTIME_SHA256 = "70354e50668e1f5950cd35145f06b2591a85b688e31dc9cb042b96b3493a802b"
BASE_REGISTRY_VERSION = "0.6.0"
CANDIDATE_REGISTRY_VERSION = "0.6.1"
ENGINE_VERSION = "0.6.2"
RELEASE_NAME = "civic_gps_lubbock_county_v0.1.json"
...
def build_release_and_bundle(output_dir: Path) -> tuple[dict, dict]:
    with tempfile.TemporaryDirectory(prefix="civic-gps-lubbock-package-") as temp_name:
        ...
        for source in sorted(onboarding.glob("*.json")):
            target = output_dir / "onboarding" / source.name
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes(source.read_bytes())

    release["meta"]["release_status"] = "RELEASE_BACKED_CURRENT"
    release["meta"]["note"] = (
        "Lubbock County is County #13 and the ninth Texas county candidate packaged through County "
        ...
        "Justice of the Peace precincts 1-4, and Constable precincts 1-4. All three district families "
        "share the official Lubbock County Precinct Boundaries layer and fail closed independently on "
        "multiple intersections. Civic-action routing and additional countywide or judicial offices "
        "remain explicitly outside v0.1 scope."
    )
    ...
    for adapter in bundle.get("district_adapters") or []:
        ...
        adapter["boundary_probe_distance_meters"] = 1
    bundle["known_gaps"] = [
        {
            "gap_id": "GAP-LUBBOCK-GPS-001",
            "status": "NOT_YET_RELEASED",
            "summary": "Lubbock County civic-action routing is not included in this geography/office candidate package.",
        },
        {
            "gap_id": "GAP-LUBBOCK-GPS-002",
        ...
        {
            "gap_id": "GAP-LUBBOCK-GPS-003",
            "status": "SHARED_GEOMETRY_VALIDATED",
            "summary": "Commissioner, Justice of the Peace, and Constable precincts share the official four-polygon District_ID layer and retain adapter-scoped fail-closed behavior.",
        },
        {
            "gap_id": "GAP-LUBBOCK-GPS-004",
            "status": "PROTECTED_PROMOTION_PENDING",
            "summary": "CG-01 through CG-08 passed; this deterministic v0.6.1 package is validated in CG-09 before the separate CG-10 protected promotion.",
        },
    ]
    if bundle.get("action_registry_files"):
        raise AssertionError("Lubbock action routing must remain absent from the candidate package")
...
def candidate_zip(base_bytes: bytes, release: dict, registry: dict) -> bytes:
    ...
    with ZipFile(io.BytesIO(base_bytes)) as base_archive, ZipFile(
        ...
        if len(names) != len(base_names) + 1:
            raise AssertionError("Lubbock package must add exactly one release entry")
        for name in names:
...
def main() -> int:
    ...
    with ZipFile(io.BytesIO(base_bytes)) as archive:
        if RELEASE_ARCHIVE_PATH in archive.namelist():
            raise AssertionError("Lubbock is already present in the production runtime")
    ...
    if any(
        row.get("adapter_id") == "ADAPTER-TX-LUBBOCK"
        for row in registry.get("bundles") or []
    ):
        raise AssertionError("Lubbock bundle already exists in production registry")
    ...
    package_path = output_dir / "civic_gps_lubbock_candidate_runtime.zip"
    package_path.write_bytes(package)
    write_json(output_dir / RELEASE_NAME, release)
    write_json(output_dir / "lubbock-registry-bundle.json", bundle)
    ...
    if len(names) != 22 or len(names) != len(set(names)):
        raise AssertionError(f"Expected 22 unique candidate entries, got {len(names)}")
    ...
    manifest = {
        ...
        "candidate_registry_canonical_sha256": candidate_registry[
        ...
        "lubbock_action_registry_files": bundle.get("action_registry_files") or [],
        "actions": "NOT_YET_RELEASED",
        "candidate_packaged": True,
        "next_gate": "CG-10",
        "stopped_before": "CG-10",
    }
    write_json(output_dir / "lubbock-candidate-package-manifest.json", manifest)
    print(json.dumps(manifest, ensure_ascii=False, sort_keys=True))
    print("LUBBOCK DETERMINISTIC CG-09 CANDIDATE PACKAGE PASS")
    return 0
