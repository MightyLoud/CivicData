#!/usr/bin/env python3
"""Live, fail-closed Bastrop County CG-04 and CG-05 proof.

The proof reads a frozen candidate and a frozen assertion manifest, verifies the
canonical roster only against official Bastrop County pages, then verifies the
shared Commissioner/JP/Constable ArcGIS layer and adapter contract. It does not
geocode or run any interior, outside-negative, boundary, package, or promotion
control.
"""
...
SCHEMA_VERSION = "civic-gps-bastrop-cg04-cg05-proof/0.1.0"
...
def _prove_roster(candidate: dict[str, Any], proof: dict[str, Any]) -> dict[str, Any]:
    ...
    allowed_hosts = {"www.bastropcounty.gov", "www.co.bastrop.tx.us"}
    ...
    def fetch_page(url: str) -> tuple[str, tuple[str, str, int]]:
        ...
        normalized = _visible_text(body)
        return url, (normalized, final_url, status)
...
def _json_fetch(url: str) -> tuple[dict[str, Any], str, int]:
    ...
    if not isinstance(value, dict) or value.get("error"):
        raise RuntimeError(f"ArcGIS error from {url}: {value.get('error') if isinstance(value, dict) else value}")
...
def _prove_gis(candidate: dict[str, Any], proof: dict[str, Any]) -> dict[str, Any]:
    ...
    _require_official(service_url, allowed_hosts)
    _require_official(str(gis["provenance_url"]), allowed_hosts)
    ...
    live_keys = sorted({str(row["attributes"][gis["district_field"]]) for row in query.get("features", [])}, key=int)
    if live_keys != list(gis["district_keys"]) or len(query.get("features", [])) != len(live_keys):
        raise ValueError(f"CG-05 live district keys/features mismatch: {live_keys}")

    provenance_body, final_provenance_url, provenance_status = _fetch(str(gis["provenance_url"]))
    ...
    if missing:
        raise ValueError(f"CG-05 provenance assertions missing: {missing}")
    ...
    return {
        ...
        "metadata_final_url": final_metadata_url,
        "metadata_http_status": metadata_status,
        "query_final_url": final_query_url,
        "query_http_status": query_status,
        "provenance_final_url": final_provenance_url,
        "provenance_http_status": provenance_status,
