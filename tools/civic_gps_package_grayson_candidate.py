#!/usr/bin/env python3
"""Build the deterministic Grayson County CG-09 candidate runtime without mutating production."""
...
BASE_REGISTRY_VERSION = "0.5.9"
CANDIDATE_REGISTRY_VERSION = "0.6.0"
ENGINE_VERSION = "0.6.2"
RELEASE_NAME = "civic_gps_grayson_county_v0.1.json"
...
def build_release_and_bundle(output_dir: Path) -> tuple[dict, dict]:
    with tempfile.TemporaryDirectory(prefix="civic-gps-grayson-package-") as temp_name:
    ...
    release["meta"]["note"] = (
        "Grayson County is County #10 and the sixth production candidate packaged through County "
        ...
        "and Justice of the Peace / Constable precincts 1-4. Current official county-directory "
        "identity controls presentation annotations and historical GIS labels. Civic-action routing "
    ...
    bundle["known_gaps"] = [
        {
            "gap_id": "GAP-GRAYSON-GPS-001",
            "status": "NOT_YET_RELEASED",
            "summary": "Grayson County civic-action routing is not included in this geography/office candidate package.",
        },
        {
            "gap_id": "GAP-GRAYSON-GPS-002",
        ...
        {
            "gap_id": "GAP-GRAYSON-GPS-003",
            "status": "SOURCE_PRECEDENCE_RESOLVED",
            "summary": "The current Grayson County elected-officials directory controls identity; appointment annotations and historical GIS labels are non-canonical metadata.",
        },
        {
            "gap_id": "GAP-GRAYSON-GPS-004",
            "status": "PROTECTED_PROMOTION_PENDING",
            "summary": "CG-01 through CG-08 passed; this deterministic v0.6.0 package is validated in CG-09 before the separate CG-10 protected promotion.",
        },
    ]
    if bundle.get("action_registry_files"):
        raise AssertionError("Grayson action routing must remain absent from the candidate package")
...
def candidate_zip(base_bytes: bytes, release: dict, registry: dict) -> bytes:
    ...
    with ZipFile(io.BytesIO(base_bytes)) as base_archive, ZipFile(
        ...
        if len(names) != len(base_names) + 1:
            raise AssertionError("Grayson package must add exactly one release entry")
        for name in names:
...
def main() -> int:
    ...
    with ZipFile(io.BytesIO(base_bytes)) as archive:
        if RELEASE_ARCHIVE_PATH in archive.namelist():
            raise AssertionError("Grayson is already present in the production runtime")
    ...
    if any(row.get("adapter_id") == "ADAPTER-TX-GRAYSON" for row in registry.get("bundles") or []):
        raise AssertionError("Grayson bundle already exists in production registry")
    ...
    package_path = output_dir / "civic_gps_grayson_candidate_runtime.zip"
    package_path.write_bytes(package)
    write_json(output_dir / RELEASE_NAME, release)
    write_json(output_dir / "grayson-registry-bundle.json", bundle)
    ...
    if len(names) != 21 or len(names) != len(set(names)):
        raise AssertionError(f"Expected 21 unique candidate entries, got {len(names)}")
    ...
    manifest = {
        ...
        "grayson_action_registry_files": bundle.get("action_registry_files") or [],
        "actions": "NOT_YET_RELEASED",
        "candidate_packaged": True,
        "next_gate": "CG-10",
        "stopped_before": "CG-10",
    }
    write_json(output_dir / "grayson-candidate-package-manifest.json", manifest)
    print(json.dumps(manifest, ensure_ascii=False, sort_keys=True))
    print("GRAYSON DETERMINISTIC CG-09 CANDIDATE PACKAGE PASS")
    return 0
