#!/usr/bin/env python3
"""Grayson County production proof for the CG-10 packaged release."""
...
def validate_packaged_release(runtime_gps: Path, registry: dict) -> dict:
    if (
        registry.get("engine_version") != "0.6.2"
        or registry.get("registry_artifact_version") != EXPECTED_REGISTRY_VERSION
    ):
        raise AssertionError(
            f"Grayson packaged regression requires engine 0.6.2 / registry "
            f"{EXPECTED_REGISTRY_VERSION}: {registry}"
        )
    ...
    if not recorded_registry_sha or recorded_registry_sha != canonical_sha(registry_without_hash):
        raise AssertionError("Grayson release registry canonical content SHA mismatch")
    grayson_bundles = [
    ...
    if set(adapter_rows) != set(ADAPTERS) or any(
        ...
        raise AssertionError(f"Grayson release adapter proof status changed: {adapter_rows}")
    gap_statuses = {
    ...
    if gap_statuses != {
        ...
        raise AssertionError(f"Grayson release known-gap states changed: {gap_statuses}")
    if any(path.name.startswith("civic_gps_action_registry_grayson") for path in runtime_gps.glob("*.json")):
        raise AssertionError("Grayson action routing must not be packaged in CG-10")

    release_path = runtime_gps / "civic_gps_grayson_county_v0.1.json"
    if not release_path.is_file():
        raise AssertionError("Grayson release file is absent")
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "artifacts/civic-gps-grayson-cg10")
    parser.add_argument(
        "--packaged",
        action="store_true",
    ...
    print("GRAYSON CG-10 RELEASE PROOF PASS" if args.packaged else "GRAYSON CG-04 THROUGH CG-08 PASS")
    return 0
