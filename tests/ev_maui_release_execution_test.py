...
from tools import ev_maui_release_execution as e
...
def source():
    ...
    for row in value["sources"]:
        row["retrieved_at_utc"] = NOW.isoformat()
    return value
...
def approved(review):
    ...
    value.update(approved=True, publication_authorized=True, expected_execution_head=HEAD,
        certified_candidate_head=CERTIFIED, candidate_pr=67, source_recheck_sha256=e.sha(e.encoded(review)),
...
class FakeAPI:
    ...
    def request(self, method, path, data=None, *, binary=False):
        ...
        if path.startswith("pulls/67/files?"):
            rows = [{"filename": p, "status": e.PATH_STATUS[p]} for p in sorted(e.ALLOWED_PATHS)]
            if self.bad_diff:
                next(x for x in rows if x["status"] == "added")["status"] = "modified"
            return rows
        ...
        if path == f"releases/{e.KAUAI_ID}":
            return {"id": e.KAUAI_ID, "target_commitish": e.KAUAI_TARGET,
                    "tag_name": "kauai-mayor-council-integration-v0.1", "draft": False}
        if path.startswith(f"releases/{e.KAUAI_ID}/assets?"):
            return [{"id": 559746136, "size": 2 if self.bad_kauai else 4593,
                     "name": "kauai-mayor-council-integration-manifest-v0.1.json",
                     "digest": "sha256:" + e.KAUAI_ASSET_SHA}]
...
class ExecutionTests(unittest.TestCase):
    ...
    def test_original_inputs_and_held_bundle(self):
        self.assertEqual(len(e.verify_local(ROOT, NOW.date())), 5596)
        self.assertEqual(e.load_json((ROOT / e.CONTRACT).read_bytes()), e.contract())
        self.assertFalse(e.approval_template()["approved"])
        self.assertEqual(e.validate_recheck((ROOT / e.RECHECK).read_bytes(), NOW)["status"], "PASS")
    ...
    def test_approval_bindings_fail_before_network(self):
        values = {"repository": "someone/else", "tag": "other-tag", "target_commit": HEAD,
                  ...
                  "kalawao_work": True, "candidate_pr": 66, "operation_id": "../escape", "approval_reference": ""}
    ...
    def test_partial_stale_future_and_extended_review_fail(self):
        ...
        with self.assertRaisesRegex(ValueError, "RECHECK_EXPIRED_OR_FUTURE"):
            e.validate_recheck(e.encoded(source()), datetime(2026, 10, 12, tzinfo=timezone.utc))
    ...
    def test_context_and_external_boundary_rejections_make_zero_writes(self):
        for field in ["bad_main", "bad_ci", "bad_diff", "bad_texas", "bad_kauai"]:
    ...
    def test_git_guard_requires_exact_ten_path_statuses(self):
        rows = "\n".join(("M\t" if path in e.MODIFIED_PATHS else "A\t") + path for path in sorted(e.ALLOWED_PATHS))
        ...
        with patch.object(e, "git", side_effect=[HEAD, "", e.BASE_TREE, rows.replace("A\t", "M\t", 1)]):
            with self.assertRaisesRegex(ValueError, "EXACT_TEN_PATHS_REQUIRED"):
                e.verify_git(ROOT, HEAD, e.BASE)

    def test_evidence_tampering_is_rejected_before_network(self):
        mutations = [
            lambda x: x["normalized_assertions"][0].update(confidence="LOW"),
            lambda x: x["normalized_assertions"][0].update(source_ids=["missing"]),
            lambda x: x["normalized_assertions"][1].update(assertion_id=x["normalized_assertions"][0]["assertion_id"]),
            lambda x: x["qa"].update(holder_parity=9),
            lambda x: x["sources"][0].update(raw_excerpts=["unbound content"]),
            lambda x: x["sources"][0].update(retrieved_at_utc=(NOW-timedelta(hours=25)).isoformat()),
            lambda x: x["sources"][0].update(retrieved_at_utc=(NOW+timedelta(seconds=1)).isoformat()),
            lambda x: x["sources"][0].update(final_url="https://untrusted.example"),
            lambda x: x["sources"][1].update(source_id=x["sources"][0]["source_id"]),
        ]
    ...
    def test_execution_shared_guard_and_immutable_inputs(self):
        from tools import ev_maui_activation_diff as guard
        output = [HEAD.encode(), "\n".join(sorted(e.ALLOWED_PATHS)).encode()]
        with patch.object(guard.subprocess, "check_output", side_effect=output), \
             patch.object(e, "verify_git", return_value={"status": "PASS"}) as exact:
            result = guard.verify_diff(ROOT, e.BASE, HEAD)
            self.assertEqual(result["mode"], "HELD_MAUI_RELEASE_EXECUTION_CANDIDATE")
            self.assertEqual(result["all_production_entries_preserved"], 6)
            exact.assert_called_once_with(ROOT, HEAD, e.BASE)
        with patch.object(guard.subprocess, "check_output", side_effect=output), \
             patch.object(e, "verify_git", side_effect=ValueError("EXACT_TEN_PATHS_REQUIRED")):
            with self.assertRaisesRegex(ValueError, "EXACT_TEN_PATHS_REQUIRED"):
                guard.verify_diff(ROOT, e.BASE, HEAD)
        with patch.object(e.manifest, "verify_inputs", side_effect=ValueError("IMMUTABLE_INPUT_DRIFT")):
            with self.assertRaisesRegex(ValueError, "IMMUTABLE_INPUT_DRIFT"):
                self.run_execution()

    def test_workflow_publication_requires_manual_main_dispatch(self):
        workflow = (ROOT / ".github/workflows/ev-maui-release-execution-candidate.yml").read_text()
        self.assertIn("default: false", workflow)
        self.assertNotIn("\n  push:", workflow)
        self.assertIn("github.event_name == 'workflow_dispatch' && github.ref == 'refs/heads/main' && inputs.execute == true", workflow)
        self.assertIn("validate_approval(approval, recheck, os.environ['EXPECTED_HEAD_SHA'])", workflow)
        compatibility = (ROOT / ".github/workflows/ev-maui-publication-candidate.yml").read_text()
        self.assertIn("verify_diff(root, os.environ['CANDIDATE_BASE_SHA'], os.environ['CANDIDATE_HEAD_SHA'])", compatibility)
